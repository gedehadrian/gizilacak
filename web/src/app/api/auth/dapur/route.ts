...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(`auth:dapur:${clientIp(req)}`, 8, 5 * 60 * 1000)) {
    return NextResponse.json(
  ...
  if (!data) {
    return NextResponse.json({ ok: false, error: "PIN dapur tidak cocok." }, { status: 401 });
  }

  await setSessionCookie("dapur", { id: data.id, nama: data.nama, kode: data.kode_dapur });
  return NextResponse.json({ ok: true, dapur: data });
}
