-- GiziLacak — skema database awal (Supabase/Postgres)
...
create table if not exists batch (
  ...
  status text not null default 'dibuat', -- dibuat | dikirim | diterima
  created_at timestamptz default now()
);
...
-- RLS terbuka untuk demo MVP (anon). Perketat sebelum produksi.
...
do $$ begin create policy "dapur_all_anon" on dapur for all using (true) with check (true); exception when duplicate_object then null; end $$;
do $$ begin create policy "sekolah_all_anon" on sekolah for all using (true) with check (true); exception when duplicate_object then null; end $$;
do $$ begin create policy "batch_all_anon" on batch for all using (true) with check (true); exception when duplicate_object then null; end $$;
do $$ begin create policy "scan_log_all_anon" on scan_log for all using (true) with check (true); exception when duplicate_object then null; end $$;
do $$ begin create policy "laporan_all_anon" on laporan for all using (true) with check (true); exception when duplicate_object then null; end $$;
